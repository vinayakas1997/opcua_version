    ...
    try:
        while True:
            for mapping in address_mappings:
                plc_address = mapping['plc']
                opcua_tag = mapping['opcua']
                ...
                try:
                    # Read from PLC
                    pack_plc_value,status,msg = fins.read(plc_address, data_type=data_type)
                    if bool_temp:
                        plc_value = bool(pack_plc_value[0])
                        bool_temp = False
                    else:
                        plc_value = pack_plc_value[0]
...
def main():
    ...
    address_mappings = [
        {'plc': '2.01', 'data_type':'int16','opcua': 'CIO201'},
        {'plc': 'C0001', 'data_type':'int16','opcua': 'iVar02'},
